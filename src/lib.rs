    ...
    if !LEAKED_CHILDREN.read().unwrap().is_empty() {
        LEAKED_CHILDREN.write().unwrap().retain_mut(|child| {
            match child.try_wait() {
                ...
                Ok(None) => true,     // retain
                Err(e) => {
                    // Ignore IO errors here unless we're running tests. It's hard to provoke
                    // these without doing something very weird (transmute, libc::waitpid,
                    // etc). If an error occurs, it's probably unrecoverable, so there's not
                    // much point in retaining the child.
                    if cfg!(test) {
